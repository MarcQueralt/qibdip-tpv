...
$cos = '<h1>' . __('Customer Invoice') . '&nbsp;' . h($customerInvoice['CustomerInvoice']['extended_number']) . '</h1>';
$cos.='<table id="document-header">';
$cos.='<tr><td style="width:20%">' . __('Customer') . ":" . '</td><td style="width:80%;">' . h($customerInvoice['Customer']['customer_name']) . '</td></tr>';
$cos.='<tr><td>' . __('Invoice Status') . ":" . '</td><td>' . h($customerInvoice['InvoiceStatus']['cus_inv_status_name']) . '</td></tr>';
$cos.='<tr><td>' . __('Invoice Date') . ":" . '</td><td>' . h($customerInvoice['CustomerInvoice']['customer_invoice_date']) . '</td></tr>';
$cos.='<tr><td>' . __('Invoice Comments') . ":" . '</td><td>' . nl2br($customerInvoice['CustomerInvoice']['customer_invoice_comments']) . '</td></tr>';
$cos.='</table>';
$tcpdf->WriteHTML($cos);
$total = 0;
$total_vat = 0;
$cos = '';
if (!empty($customerInvoice['CustomerInvoiceLine'])):
    ...
    $cos.='<th style="width:70%">' . __('Article') . '</th>';
    $cos.='<th style="width:15%;text-align:right;">' . __('Amount') . '</th>';
    $cos.='</tr>';
    $i = 0;
    foreach ($customerInvoice['CustomerInvoiceLine'] as $customerInvoiceLine):
        $total+=$customerInvoiceLine['customer_invoice_line_amount'];
        $total_vat+=$customerInvoiceLine['customer_invoice_line_vat'];
        $cos.=(1 == $i++ % 2) ? '<tr style="background-color:#eee;">' : '<tr>';
        $cos.='<td>' . h($customerInvoiceLine['customer_invoice_line_number']) . '</td>';
        $cos.='<td>' . qibdipTPV_prettyOrderLineType($customerInvoiceLine['customer_invoice_line_type']) . '</td>';
        $cos.='<td>';
        if ('A' == $customerInvoiceLine['customer_invoice_line_type']):
            $cos.=h($customerInvoiceLine['customer_invoice_line_text']);
        else:
            $cos.=nl2br($customerInvoiceLine['customer_invoice_line_description']);
        endif;
        $cos.='</td>';
        $cos.='<td style = "text-align:right;">' . $this->Number->currency($customerInvoiceLine['customer_invoice_line_amount'], 'QBD') . '</td>';
        $cos.='</tr>';
    endforeach;
    $cos.='<tr style="background-color:#aaa;">';
    $cos.='<td colspan="3" style="text-align:right;color:white;font-weight:bold;">' . __('Total Amount') . '</td>';
    $cos.='<td style="text-align:right;color:white;font-weight:bold;">' . $this->Number->currency($total, 'QBD') . '</td>';
    $cos.='</tr>';
    $cos.='<tr style="background-color:#aaa;">';
    $cos.='<td colspan="3" style="text-align:right;color:white;font-weight:bold;">' . __('Total VAT') . '</td>';
    $cos.='<td style="text-align:right;color:white;font-weight:bold;">' . $this->Number->currency($total_vat, 'QBD') . '</td>';
    $cos.='</tr>';
    $cos.='<tr style="background-color:#aaa;">';
    $cos.='<td colspan="3" style="text-align:right;font-size:1.3em;color:white;font-weight:bold;">' . __('Total') . '</td>';
...
$tcpdf->WriteHTML($cos);
//$tcpdf->WriteHTML('<pre>' . print_r($customerInvoice, true) . '</pre>');
echo $tcpdf->Output(__('Invoice') . '-' . $customerInvoice['CustomerInvoice']['id'] . '.pdf', 'D');
?>